...
(define (big-add1 x y co)
  (cond
    ...
    [else
     (cond
       [(>= (+(car x)(car y)) MAX_BLOCK) (cons (+(- (+(car x)(car y)) MAX_BLOCK) co)(big-add1 (cdr x) (cdr y) 1))]
       [else
...
(define (big-subtract1 x y borrow)
  ...
  (cond
    ...
    [else
     [cond
           [(< (-(car x)(car y)) 0) (cons(-(+(car x) MAX_BLOCK)(car y)) (big-subtract1 (cdr x) (cdr y) 1))]
       [else
...
(define (big-eq x y)
  ...
  ;; --- YOUR CODE HERE ---
  ;;
  (cond [(empty? x)(error "empty")])
  (cond [(empty? y)(error "empty")])
  (if(equal? x y)
     'true
     'false)
  )
...
(define (big-multiply x y)
  ...
  [cond
    ((equal? (big-eq '(2) y) 'false)(big-add x (big-multiply x (big-dec y))))
    [else
     (big-add x x)]])
...
(define (big-power-of x y)
  ...
  [cond
    ((equal? (big-eq '(2) y) 'false)(big-multiply x (big-power-of x (big-dec y))))
    [else
     (big-multiply x x)]])
